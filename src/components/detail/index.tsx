...
import { DetailDoa } from "src/interface";
import ModalKeterangan from "../modalKeterangan";

const Detail = ({ item }: DetailDoa) => {
  return (
    <Flex
      flexDir="column"
      textAlign="center"
    ...
    >
      ...
      {item.keterangan && <ModalKeterangan keterangan={item.keterangan} />}
      {item.id_doa === "20" && (
        <Text fontSize="lg" mt="2" letterSpacing="wide">
          {item.id_doa === "20" && item.keterangan[0]}
        </Text>
      )}
      <Text fontWeight="extrabold" mt="4" fontSize="4xl" letterSpacing="widest">
        {item.id_doa === "20" ? item.keterangan[1] : item.lafal}
      </Text>
      <Text mt="1" fontSize="lg" fontWeight="semibold" letterSpacing="wide">
